...
describe("Identity Password Reset Feature - wrapper", () => {
	...
	it("renders", () => {
		const wrapper = shallow(<ResetPassword customFields={{ successActionURL }} />);
...
describe("Identity Password Reset Feature", () => {
	...
	it("shows submit form", () => {
		...
		expect(wrapper.text().includes(phrases.t("identity-block.reset-password-instruction"))).toBe(
			true
		);
	...
	it("updates the page on submit and redirects the user to login when done", async () => {
		...
		wrapper.update();
		expect(
			wrapper.text().includes(phrases.t("identity-block.reset-password-headline-submitted"))
		).toBe(true);
		expect(
			wrapper.text().includes(phrases.t("identity-block.reset-password-instruction-submitted"))
		).toBe(true);
		expect(
			wrapper.text().includes(phrases.t("identity-block.reset-password-submit-submitted"))
		).toBe(true);
		await act(async () => {
			wrapper.find("form").simulate("submit");
		});

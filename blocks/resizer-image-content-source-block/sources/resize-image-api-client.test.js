...
describe("the resizer image api client source block", () => {
	...
	it("should call getResizedImageData if image and domain match no environment domains", () => {
		...
		contentSource.fetch({
			raw_image_url: "http://themes.images.arcpublishing.com/image.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(1);

		contentSource.fetch({
			raw_image_url: "http://images.arcpublishing.com/image.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(2);

		// 's3.amazonaws.com/arc-authors/', is a default allowed domain
		contentSource.fetch({
			raw_image_url: "https://s3.amazonaws.com/arc-authors/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(3);

		contentSource.fetch({
			raw_image_url: "https://s3.amazonaws.com/arcauthors/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(3);
	...
	it("should call getResizedImageData if image and domain match with environment domains", () => {
		...
		contentSource.fetch({
			raw_image_url: "https://my-custom-domain.com/image.jpg",
		});
	...
	it("allows default arc domains to be called", () => {
		...
		contentSource.fetch({
			raw_image_url: "https://static.themebuilder.aws.arc.pub/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(4);

		contentSource.fetch({
			raw_image_url:
				"https://themebuilder-api-uploads-ap-northeast-1.s3.amazonaws.com/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(5);

		contentSource.fetch({
			raw_image_url:
				"https://themebuilder-api-uploads-eu-central-1.s3.amazonaws.com/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(6);

		contentSource.fetch({
			raw_image_url: "https://themebuilder-api-uploads-us-east-1.s3.amazonaws.com/marty-mcfly.jpg",
		});
		expect(mockFn.mock.calls.length).toBe(7);

		contentSource.fetch({
			raw_image_url: "https://themebuilder-api-uploads.s3.amazonaws.com/marty-mcfly.jpg",
		});

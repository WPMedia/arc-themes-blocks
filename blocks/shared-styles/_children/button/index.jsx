...
function renderButtonContents(
	...
	const buttonText = isHTMLText ? <span dangerouslySetInnerHTML={{ __html: text }} /> : text;

	switch (matchedButtonType) {
		case BUTTON_TYPES.LABEL_AND_TWO_ICONS:
			return (
				<>
					<span className="xpmedia-button--left-icon-container">{iconComponent}</span>
					{buttonText}
					<span className="xpmedia-button--right-icon-container">{secondaryIconComponent}</span>
				</>
			);
		...
		case BUTTON_TYPES.LABEL_AND_RIGHT_ICON:
			return (
				<>
					{buttonText}
					<span className="xpmedia-button--right-icon-container">{iconComponent}</span>
				</>
			);
	...
	return (
		<StyledDynamicButton
			...
			className={`xpmedia-button ${matchedButtonSizeClass}${
				fullWidth ? " xpmedia-button--full-width" : ""
...
Button.propTypes = {
	...
	secondaryIconType: PropTypes.oneOf([
		"user",
		"chevron-up",
		"chevron-down",
		"hamburger-menu",

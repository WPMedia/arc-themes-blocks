...
describe("SubscriptionDialog", () => {
	it("renders with minimal required properties", () => {
		const wrapper = mount(<SubscriptionDialog linkText="Log in" linkUrl="/" />);
	...
	it("does not render if other required action part is missing", () => {
		const wrapper = mount(<SubscriptionDialog linkText="Log in" linkUrl="/" actionUrl="/" />);
	...
	it("renders the headline", () => {
		...
		expect(wrapper.find(".xpmedia-subscription-dialog-headline").at(0).text()).toEqual(
			"Headline 1"
		);
	});
	...
	it("renders the subheadlines", () => {
		...
		expect(wrapper.find(".xpmedia-subscription-dialog-subheadline").at(0).text()).toEqual(
			"Headline 2"
		);
	});
	...
	it("renders the reason", () => {
		...
		expect(wrapper.find(".xpmedia-subscription-dialog-reason-prompt").at(0).text()).toEqual(
			"You need to do this."
		);
	});
	...
	it("renders the link prompt text", () => {
		...
		expect(wrapper.find(".xpmedia-subscription-dialog-link-prompt-pre-link").at(0).text()).toEqual(
			"You should log in."
		);
	});
});

...
export const capitalize = (string) =>
	!string ? string : [String(string).charAt(0).toUpperCase(), String(string).substring(1)].join("");
...
export const generateNavComponentPropTypes = () => {
	...
	Object.keys(WIDGET_CONFIG).forEach((cfgKey) => {
		const { sections, options, slotCounts } = WIDGET_CONFIG[cfgKey];
...
export const hasUserConfiguredNavItems = (customFields) => {
	...
	const { slotCounts, sections: navBarSections } = WIDGET_CONFIG[PLACEMENT_AREAS.NAV_BAR];

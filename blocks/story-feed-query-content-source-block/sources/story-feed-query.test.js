...
describe('the story feed query block', () => {
  ...
  describe('when a query is provided', () => {
    expect(storyQuery.resolve({ 'arc-site': 'my-site', query: 'queryqueryquery' })).toEqual('/content/v4/search/published?q=queryqueryquery&website=my-site&size=8&from=0&sort=display_date:desc');
  });

  describe('when a query is NOT provided', () => {
    expect(storyQuery.resolve({ 'arc-site': 'my-site' })).toEqual('/content/v4/search/published?q=*&website=my-site&size=8&from=0&sort=display_date:desc');
  });

  describe('when a size is provided', () => {
    expect(storyQuery.resolve({ 'arc-site': 'my-site', size: 4 })).toEqual('/content/v4/search/published?q=*&website=my-site&size=4&from=0&sort=display_date:desc');
  });
  ...
  describe('when an offset is provided', () => {
    expect(storyQuery.resolve({ 'arc-site': 'my-site', offset: 12 })).toEqual('/content/v4/search/published?q=*&website=my-site&size=8&from=12&sort=display_date:desc');
  });

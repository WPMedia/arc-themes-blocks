...
const ProductPriceDetails = ({
	...
	if (details?.items?.length) {
		return details?.items?.map((item) => (
			<Stack as="div" key={`${id}-productDetails`}  className={`${className}__orderCard--productPrice`}>
...
const OrderInformation = ({
	...
	return (
		<div className={showBorder ? `${className}__orderCard-borderOn` : undefined}>
			<section key={id} className={`${className}__orderCard`}>
				<ProductPriceDetails
					key={id}
...
OrderInformation.propTypes = {
	...
	showOfferURL: PropTypes.boolean,
	showPriceDescription: PropTypes.boolean,
	showProductFeatures: PropTypes.boolean,

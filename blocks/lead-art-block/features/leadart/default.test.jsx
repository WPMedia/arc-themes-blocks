...
describe("LeadArt", () => {
	...
	it("uses english phrases if no locale available", () => {
		getProperties.mockImplementation(() => ({ locale: undefined }));
	...
	it("uses translated gallery phrase if no custom field value provided", () => {
		...
		const wrapper = shallow(
			<LeadArt
				arcSite="the-sun"
				globalContent={globalContent}
				customFields={customFieldNullButton}
			/>
		);
	...
	it("creates lightbox", () => {
		...
		wrapper.instance().imgRef = {
			current: {
				querySelector: jest.fn().mockReturnValue({ dataset: { lightbox: "testLightbox" } }),
			},
		};
	...
	it(" lightbox returns empty string if no imgElm ", () => {
		...
		wrapper.instance().imgRef = {
			current: { querySelector: jest.fn().mockReturnValue(null) },
		};
		const result = wrapper.instance().lightboxImgHandler();

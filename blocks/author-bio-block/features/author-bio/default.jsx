...
const AuthorItem = ({ author }) => {
	...
	// Case 1: If resizedAuth contains a `hash` property, construct a new `auth` object for ansImage
	// Example: resizedAuth = { hash: "abc123" }. This handles signed URLs generated by the signing service
	if (resizedAuth?.hash) {
		updatedAnsImage = {
			...ansImage,
		...
		// Case 2: resizedAuth exists but does not contain a hash — fallback to using available auth and URL
		// Example: resizedAuth = { 2: "token" } or { 1: "token", 2: "token2" }
		if (ansImage?.auth && ansImage?.url) {
			// Case 2a: use ansImage with resizedAuth
			updatedAnsImage = {
				...ansImage,
				auth: resizedAuth,
			};
		} else if (originalAnsImage?.auth && originalAnsImage?.url) {
			// Case 2b: fallback to originalAnsImage if ansImage is not usable
			updatedAnsImage = {
				...originalAnsImage,
				auth: resizedAuth,
			};
